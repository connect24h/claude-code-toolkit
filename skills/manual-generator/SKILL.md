...
- 直接実行: `npx tsx $HOME/manual-generator/src/index.ts <config.yaml>`
...
```bash
# 直接指定
APP_USERNAME=user APP_PASSWORD=pass npx tsx $HOME/manual-generator/src/index.ts config.yaml

# SOPS連携
source $HOME/.claude/scripts/lib/decrypt-env.sh /path/to/.env.enc
npx tsx $HOME/manual-generator/src/index.ts config.yaml
```

### Step 3: 実行

```bash
cd $HOME/manual-generator
...
- プロジェクト: `$HOME/manual-generator/`
- サンプル設定: `$HOME/manual-generator/examples/sample-config.yaml`
- テスト: `cd $HOME/manual-generator && npm test`
