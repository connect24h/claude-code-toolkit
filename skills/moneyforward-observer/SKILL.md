...
```bash
# 1. ディレクトリ作成
mkdir -p $HOME/moneyforward/{config,snapshots,reports}

# 2. 認証情報設定
cat > $HOME/moneyforward/config/credentials.env << 'EOF'
MF_EMAIL=your-email@gmail.com
MF_PASSWORD=your_password_here
EOF

chmod 600 $HOME/moneyforward/config/credentials.env
```
...
`$HOME/moneyforward/scripts/observe.py` を使用。
